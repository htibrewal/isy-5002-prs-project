...
import requests
import json
from sg_areas import get_areas, get_subzones, Region
...
def get_singapore_locations():
   ...
   # Get all areas within Singapore
    all_areas = get_areas()

   # Get all subzones within Singapore (more granular than areas)
    all_subzones = get_subzones()

   # Get areas within specific regions (e.g., Central Region)
    central_areas = [(area['coordinates']['latitude'], area['coordinates']['longitude']) for area in all_areas.values()]
    central_subzones = [(subzone['coordinates']['latitude'], subzone['coordinates']['longitude']) for subzone in all_subzones.values() if subzone['region'] == Region.CENTRAL]
    coordinates = central_areas + central_subzones
